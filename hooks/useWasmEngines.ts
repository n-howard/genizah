  ...
  useEffect(() => {
    ...
    async function loadEngines() {
      try {
        ...
        // 2. Load WebR cleanly from CDN (bypassing local worker missing 404s)
        ...
        const webr = new WebR({
          baseUrl: cdnUrl,
          serviceWorkerUrl: `${cdnUrl}webr-serviceworker.js`,
          channelType: 3 // PostMessage channel (No Service Worker registration required)
        });
